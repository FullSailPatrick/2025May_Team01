...
class Level02GamePlayScreen : AppCompatActivity(), GestureDetector.OnGestureListener {
    ...
    val FlashCardQuestionsAnswers2 = listOf (
        FlashCard("Why is water denser at 4°C than at 0°C", "Hydrogen bonding causes expansion below 4°C"),
        FlashCard("What principle states you can't simultaneously know a particle’s position and momentum exactly", "The Heisenberg Uncertainty Principle"),
        FlashCard("Why do we age?", "DNA damage, telomere shortening, and cellular senescence."),
        FlashCard("What's 1 + 1?", "2"),
        FlashCard("What happens to entropy in a closed system over time?", "It increases"),
        FlashCard("In computer science, what measures algorithm efficiency?", "Big O Notation"),
        FlashCard("What's 1 + 2 + 3 ?", "6"),
        FlashCard("Are you smarter than a 5th grader?", "Maybe..."),
    )

    //keeps track of what index you're on --> I NEED TO FIGURE THIS PART OUT!!!!
    fun goToNextFlashCard() {

        var FlashCardHolder = FlashCardQuestionsAnswers2[index]
        index += 1

        if(index == FlashCardQuestionsAnswers2.size)
        {
    ...
    fun LoadQuestion() {
        FlashCardText.setText(FlashCardQuestionsAnswers2[index].questions)
    }

    fun LoadAnswer() {
        FlashCardText.setText(FlashCardQuestionsAnswers2[index].answers)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.lvl02gameplay_screen)
        ...
        val timer = object: CountDownTimer(20000, 1000) {
            ...
            override fun onFinish() {
                val intent = Intent(this@Level02GamePlayScreen, PlayerResultsScreen::class.java)
                intent.putExtra("correctAnswers", correctAnswers)
                intent.putExtra("totalAnswers", TotalAnswers)
                startActivity(intent)
                finish()
